...
#' @description A tool to assist in the visualization of the results from INDEED functions patial_corr() 
#' and non_partial_corr(). The size and the color of each node can be adjusted by users to represent either 
...
#' @param results This is the result from the calling either partial_corr() or non_partial_corr()
...
#' @param nodecolor This parameter determines what color each node will be based on a yellow ro red color gradient.
...
#'
#' @examples 		result1 = non_partial_corr(data=Met_GU,class_label = Met_Group_GU,id=Met_name_GU,
...
#'
#' @return A visual dipiction of the network resulting from INDEED functions patial_corr() or non_partial_corr()
#' @import igraph
